...
public class LicensesWpfCustomersSyncControllerTests : IClassFixture<ApiFactory>
{
    ...
    private sealed record PullItem(
        Guid Id, string Platform, string Username, string? FullName,
        string? Phone, string? Address, DateTimeOffset UpdatedAt, bool SmsConsent);

    [Fact]
    public async Task SmsConsent_round_trips_through_sync_and_pull()
    {
        var (client, _, licenseId) = await SetupAsync();
        var idConsent = Guid.NewGuid();
        var idNoConsent = Guid.NewGuid();
        ...
        var resp = await client.PostAsJsonAsync(
            ...
            new
            {
                customers = new[]
                {
                    new { id = idConsent, platform = "youtube", username = "consented",
                          fullName = (string?)null, phone = "+905001112233", address = (string?)null,
                          updatedAt = DateTimeOffset.UtcNow, smsConsent = true },
                    new { id = idNoConsent, platform = "youtube", username = "notconsented",
                          fullName = (string?)null, phone = "+905004445566", address = (string?)null,
                          updatedAt = DateTimeOffset.UtcNow, smsConsent = false },
                }
            });
        resp.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<LicenseDbContext>();
            (await db.WpfCustomerProjections.FirstAsync(p => p.Id == idConsent)).SmsConsent.Should().BeTrue();
            (await db.WpfCustomerProjections.FirstAsync(p => p.Id == idNoConsent)).SmsConsent.Should().BeFalse();
        }

        // And flows back through the pull endpoint
        var pull = await client.GetFromJsonAsync<List<PullItem>>(
            $"/api/v1/licenses/{licenseId}/wpf-customers/since?since={Uri.EscapeDataString(DateTimeOffset.UnixEpoch.ToString("o"))}");
        pull!.Single(p => p.Id == idConsent).SmsConsent.Should().BeTrue();
        pull.Single(p => p.Id == idNoConsent).SmsConsent.Should().BeFalse();
    }

    [Fact]
    public async Task SmsConsent_defaults_false_when_field_omitted()
    {
        var (client, _, licenseId) = await SetupAsync();
        var id = Guid.NewGuid();

        // Legacy WPF clients that don't send smsConsent → default false (geriye uyumlu)
        var resp = await client.PostAsJsonAsync(
            $"/api/v1/licenses/{licenseId}/wpf-customers/sync",
            new { customers = new[] { MakeSyncItem(id, "youtube", "legacy") } });
        resp.StatusCode.Should().Be(HttpStatusCode.OK);

        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<LicenseDbContext>();
        (await db.WpfCustomerProjections.FirstAsync(p => p.Id == id)).SmsConsent.Should().BeFalse();
    }
}
