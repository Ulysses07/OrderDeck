...
/// 2. "pending" + oluşturulalı belli süre geçmiş + bayat/boş ClaimedAt —
///    kampanya yazıldı ama Enqueue hiç gerçekleşmedi (Create ile Enqueue
///    arasında çökme, F09'un kayıp-enqueue boşluğu) YA DA devam ettirmenin
///    enqueue'su koptu. Yeniden kuyruğa almak boşluğu kapatır.
...
public sealed class SmsCampaignRecoveryJob
{
    ...
    /// tamamlar. Sınıf doc'undaki gerekçe geçerli — kampanya gönderime
    /// SOKULMAZ, burada bitirilir.
    ///
    ...
    private async Task CompleteStrandedAsync(CancellationToken ct)
    {
        ...
        // gitmemiş SMS'leri gitmiş saymak olurdu.
        ...
        foreach (var campaign in stranded)
        {
            ...
            _log.LogWarning(
                "SmsCampaignRecoveryJob: completing stranded paused campaign {Id}",
                campaign.Id);
